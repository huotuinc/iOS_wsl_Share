...
- (void)resetBtnFrame{
    ...
    if (self.titlesArr.count>0&&self.framesArr.count>0) {
        ...
        switch ((int)rect.origin.y) {
            //任务详情
            case 1000:
                self.btnX.constant = (ScreenWidth - ScreenWidth / 3 * 2) / 2 - 20;
                self.btnY.constant = ScreenHeight - 54;
                self.btnW.constant = ScreenWidth / 3 * 2 + 40;
                self.btnH.constant = 54;
                break;
            //左侧抽屉
            case 1001:
                self.btnX.constant = 0;
                self.btnY.constant = ScreenHeight / 5 + 120;
                self.btnW.constant = 180;
                self.btnH.constant = 60;
